define(function(require, exports, module) {
  ...
  function startup(worker) {
     ...
     * from the UI thread. Signals that some work needs to occur. Will post
     * back to the main thread with patch and node transform results.
     *
    ...
    worker.onmessage = function(e) {
      ...
      // If an `oldTree` was provided by the UI thread, use that in place of
      // the current `oldTree`.
      if (data.oldTree) { oldTree = data.oldTree; }

      // If the `newTree` was provided to the worker, use that instead of
      // trying to create one from HTML source.
      if (data.newTree) { newTree = data.newTree; }

      // If no `newTree` was provided, we'll have to try and create one from
      // the HTML source provided.
