define(function(require, exports, module) {
  ...
  var createWorker = require('../worker/create').createWorker;
  var hasWorker = require('../worker/create').hasWorker;
  ...
  var processPatches = require('../patch/process');
  var makeNode = require('./make');
  var makeElement = require('../element/make');
  var syncNode = require('./sync');
  var TreeCache = require('./tree');
  ...
  function completeWorkerRender(element, elementMeta) {
    return function(ev) {
      ...
      var completeRender = function() {
        ...
        // This is designed to handle use cases where renders are being
        // hammered or when transitions are used with Promises.
  ...
  function patchNode(element, newHTML, options) {
    ...
    // FIXME Temporary fix, as this should be early returning. The problem
    // is that occasionally an element mixup occurs which triggers an error
    // and doesn't complete the render cycle.
    ...
    if (
      // If the operation is `innerHTML`, and the current element's contents
      // have changed since the last render loop, recalculate the tree.
      (options.inner && elementMeta._innerHTML !== element.innerHTML) ||

      // If the operation is `outerHTML`, and the current element's contents
      // have changed since the last render loop, recalculate the tree.
    ...
    if (options.enableWorker && hasWorker && worker) {
      ...
      if (typeof newHTML !== 'string') {
        ...
        // Wait for the worker to finish processing and then apply the
        // patchset.
      ...
      var completeRender = function() {
        ...
        // TODO Update this comment and/or refactor to use the same as the
        // Worker.
        nextRender();
      };
